...
class GpxExporter {
  /// Exportiert alle GPS-Fixes eines bestimmten Einsatzzeitraums.
  static Future<String> exportDeploymentGpx({
    ...
    final durationMin = (endMs - startMs) ~/ 60000;

    final metaName = trupp.isNotEmpty ? 'Trupp $trupp ($issi)' : 'ISSI $issi';
    final metaDesc = [
      'Einsatz ${_tsDisplay(start)} (${durationMin} min)',
    ...
    buf.writeln('    <time>${start.toIso8601String()}</time>');
    buf.writeln('  </metadata>');
    buf.writeln('  <trk>');
    buf.writeln('    <name>${_xml(metaName)}</name>');
    buf.writeln('    <trkseg>');
    ...
    final fileName = 'truppapp_${_safe(issi)}_einsatz_${_tsFile(start)}.gpx';
  ...
  static String _tsDisplay(DateTime t) {
    final d = t.toLocal();
